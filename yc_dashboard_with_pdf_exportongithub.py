...
@st.cache_data
def load_data(path: str = "revenue.csv") -> pd.DataFrame:
    df = pd.read_csv(path)
    ...
    df["Sold"] = pd.to_numeric(df.get("Sold"), errors="coerce")
    df["orderdate_year"] = pd.to_numeric(df.get("orderdate_year"), errors="coerce")
    df["orderdate"] = pd.to_datetime(df["orderdate_month"] + "-" + df["orderdate_year"].astype(str), format="%B-%Y", errors="coerce")

    df["orderdate_month"] = df["orderdate"].dt.month_name()
    df["month_num"] = df["orderdate"].dt.month
    df["Category"] = df["Category"].astype(str).str.strip()
    df["year"] = df["orderdate_year"]

    return df.dropna(
        subset=["Net_Sales", "Category", "orderdate", "month_num", "year"]
    ).copy()


@st.cache_data
def load_nonrevenue_data(path: str = "nonreveneu.csv") -> pd.DataFrame:
    df = pd.read_csv(path, header=None)
    df.columns = ["Category", "Amount", "Count", "Month", "Year"]
    ...
    df["orderdate"] = pd.to_datetime(df["Month"] + "-" + df["Year"].astype(str), format="%B-%Y", errors="coerce")
    df["month_num"] = df["orderdate"].dt.month
    
    return df.dropna(
        subset=["Amount", "Count", "Year", "orderdate"]
    ).copy()
...
def get_nonrevenue_metric_label(category: str) -> str:
    label_map = {
        "Donation": "transactions",
        "Membership Lifetime": "members",
        "Membership Renewal": "renewals",
        "VFW Lifetime (Paid In Full)": "members",
        "VFW New Member Yearly": "sign-ups",
        "VFW New Member/Yearly (Not Lifetime)": "sign-ups",
    }
    return label_map.get(category, "count")
...
def build_insights(
    ...
    nonrevenue_metrics: dict[str, dict[str, float | int]],
) -> list[str]:
    insights = []
    ...
    nonrevenue_ranked = sorted(
        nonrevenue_metrics.items(),
        key=lambda item: item[1]["amount"],
        reverse=True,
    )
    top_nonrevenue = next((item for item in nonrevenue_ranked if item[1]["amount"] > 0), None)
    if top_nonrevenue is not None:
        category, metrics = top_nonrevenue
        metric_label = get_nonrevenue_metric_label(category)
        insights.append(
            f"**{category}** leads non-revenue activity with **${metrics['amount']:,.0f}** across **{format_quantity(metrics['count'])} {metric_label}**."
        )

    donation_metrics = nonrevenue_metrics.get("Donation")
    if donation_metrics and donation_metrics["amount"] > 0:
        insights.append(
            f"Donations contributed **${donation_metrics['amount']:,.0f}** from **{format_quantity(donation_metrics['count'])} transactions**, reinforcing member support beyond bar sales."
        )
...
def generate_pdf_report(
    ...
    story.append(
        Paragraph(
            f"Prepared by YC Analytics | Exported view: {selected_year} | Month filter: {selected_month} | Category filter: {selected_category}",
            subtitle_style,
        )
    )
    ...
    nonrevenue_rows = [["Metric", "Amount", "Volume"]]
    for category, metrics in nonrevenue_metrics.items():
        nonrevenue_rows.append(
            [
                category,
                f"${metrics['amount']:,.0f}",
                f"{format_quantity(metrics['count'])} {get_nonrevenue_metric_label(category)}",
            ]
        )

    nonrevenue_table = Table(nonrevenue_rows, colWidths=[2.75 * inch, 1.2 * inch, 2.0 * inch])
    nonrevenue_table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#1f4e92")),
                ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
    ...
    story.append(Paragraph("Membership & Donation Summary", heading_style))
    story.append(nonrevenue_table)
    story.append(Spacer(1, 0.16 * inch))
...
nonrevenue_df = load_nonrevenue_data()
...
with header_right:
    ...
    with filter_cols[1]:
        year_months = [
            ...
            if month in df.loc[df["year"] == int(selected_year), "orderdate_month"].dropna().unique()
        ]
        st.markdown(
            '<div style="font-size:18px; font-weight:800; color:#294d7d; margin-bottom:6px;">Month</div>',
            unsafe_allow_html=True
        )
        selected_month = st.selectbox("Month", ["All"] + year_months, label_visibility="collapsed")
    with filter_cols[2]:
        st.markdown(
...
if selected_month != "All":
    filtered_df = filtered_df[filtered_df["orderdate_month"] == selected_month]
if selected_category != "All":
    filtered_df = filtered_df[filtered_df["Category"] == selected_category]

prev_df = df[df["year"] == int(selected_year) - 1].copy()
if selected_month != "All":
    prev_df = prev_df[prev_df["orderdate_month"] == selected_month]
if selected_category != "All":
...
nonrevenue_filtered = nonrevenue_df[nonrevenue_df["Year"] == int(selected_year)].copy()
if selected_month != "All":
    nonrevenue_filtered = nonrevenue_filtered[nonrevenue_filtered["Month"] == selected_month]

# Define categories for KPIs
kpi_categories = [
    "Donation",
    "Membership Lifetime",
    "Membership Renewal",
    "VFW Lifetime (Paid In Full)",
    "VFW New Member Yearly",
    "VFW New Member/Yearly (Not Lifetime)"
]

# Calculate KPI values for each category
kpi_values = {}
for category in kpi_categories:
    category_data = nonrevenue_filtered[nonrevenue_filtered["Category"] == category]
    kpi_values[category] = {
        "amount": category_data["Amount"].sum() if not category_data.empty else 0,
        "count": category_data["Count"].sum() if not category_data.empty else 0,
    }

st.markdown("### 📊 Membership & Donation Metrics")
...
with k5:
    st.markdown(
        f"""
        <div class="kpi-card">
            <div class="kpi-label">DONATION</div>
            <div class="kpi-value">${kpi_values['Donation']['amount']:,.0f}</div>
            <span class="kpi-quantity">{format_quantity(kpi_values['Donation']['count'])} transactions</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
with k6:
    st.markdown(
        f"""
        <div class="kpi-card">
            <div class="kpi-label">MEMBERSHIP LIFETIME</div>
            <div class="kpi-value">${kpi_values['Membership Lifetime']['amount']:,.0f}</div>
            <span class="kpi-quantity">{format_quantity(kpi_values['Membership Lifetime']['count'])} members</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
with k7:
    st.markdown(
        f"""
        <div class="kpi-card">
            <div class="kpi-label">MEMBERSHIP RENEWAL</div>
            <div class="kpi-value">${kpi_values['Membership Renewal']['amount']:,.0f}</div>
            <span class="kpi-quantity">{format_quantity(kpi_values['Membership Renewal']['count'])} renewals</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
with k8:
    st.markdown(
        f"""
        <div class="kpi-card">
            <div class="kpi-label">VFW LIFETIME (PAID IN FULL)</div>
            <div class="kpi-value">${kpi_values['VFW Lifetime (Paid In Full)']['amount']:,.0f}</div>
            <span class="kpi-quantity">{format_quantity(kpi_values['VFW Lifetime (Paid In Full)']['count'])} members</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
with k9:
    st.markdown(
        f"""
        <div class="kpi-card">
            <div class="kpi-label">VFW NEW MEMBER YEARLY</div>
            <div class="kpi-value">${kpi_values['VFW New Member Yearly']['amount']:,.0f}</div>
            <span class="kpi-quantity">{format_quantity(kpi_values['VFW New Member Yearly']['count'])} sign-ups</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
with k10:
    st.markdown(
        f"""
        <div class="kpi-card">
            <div class="kpi-label">VFW NEW/YEARLY (NOT LIFETIME)</div>
            <div class="kpi-value">${kpi_values['VFW New Member/Yearly (Not Lifetime)']['amount']:,.0f}</div>
            <span class="kpi-quantity">{format_quantity(kpi_values['VFW New Member/Yearly (Not Lifetime)']['count'])} sign-ups</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
insights = build_insights(category_sales, monthly_revenue, growth_pct, kpi_values)
#Insights Title
