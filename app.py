...
with header_right:
    ...
    with filter_cols[0]:
        st.markdown(
            '<div style="font-size:18px; font-weight:800; color:#294d7d; margin-bottom:6px;">Year</div>',
            unsafe_allow_html=True
        )
        selected_year = st.selectbox("Year", years, index=len(years) - 1, label_visibility="collapsed")
    with filter_cols[1]:
        year_months = [
...
st.markdown("### 📊 Donations & Membership Metrics")
...
with k7:
    st.markdown(
        f"""
        <div class="kpi-card">
            <div class="kpi-label">MEMBERSHIP YEARLY</div>
            <div class="kpi-value">${kpi_values['Membership Yearly']['amount']:,.0f}</div>
            <span class="kpi-quantity">{format_quantity(kpi_values['Membership Yearly']['count'])} members</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
